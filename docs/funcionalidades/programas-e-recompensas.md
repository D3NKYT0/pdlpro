...
A pescaria tem uma única interface em Jogos (`Pescaria`), incluindo vara, experiência, iscas, coleção e últimos lançamentos. O endereço antigo `/panel/rewards?tab=fishing` redireciona para a aba Pescaria em Jogos, sem alterar o progresso. O lago mostra cardume em pixel art com esteiras na água; o lançamento cai a boia, a fisgada afunda a linha e o troféu salta (ou foge) com a sprite da espécie (`images/games/fish-*.webp`). A tela segue o módulo dos outros jogos: palco e HUD à esquerda, loja e últimos lances à direita, coleção em grade embaixo. A coleção usa as mesmas artes; espécies novas caem na sprite da raridade.
