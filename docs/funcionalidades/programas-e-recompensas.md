...
`/panel/games` usa arte do tema ativo (`images/games/*`, tokens `--theme-art-games-*`) e anima o palco de cada minigame (tambor da roleta, abertura de baú, dado/slots, lago e combate). Os nomes de vitrine são **Roda da Fortuna**, **Baús Encantados**, **Mesa da Taverna**, **Pescaria** e **Arena das Feras**. Pacotes podem remapear esses arquivos no mapa `assets` sem alterar o HTML. A roleta pede o giro na hora: o tambor começa rápido, desacelera perto do fim e só revela o item no centro (com explosão de fogos no acerto) após a animação, sem toast. Se a roda não parar em prêmio, o círculo fica vermelho, treme e mostra um X.

Os **Baús Encantados** funcionam diferente da roleta: o **item em mira** já está em um dos pacotes. Quem compra leva o item se abrir todos — a sorte só decide se sai no primeiro ou no último. A compra do baú é em reais e libera N pacotes; **abrir** um pacote consome 1 ficha. O resto do catálogo preenche os outros pacotes. O baú do card treme e, em seguida, um modal no centro troca os frames (fechado → entreaberto → aberto) e revela o item, sem toast. Os baús default do autoconfig separam o catálogo por raridade (comum sem joia de boss; lendário com Ring of Baium e Blessed Enchant S). A arte do baú é pixel art da paleta do painel (`images/games/box-*.webp`), não foto.
