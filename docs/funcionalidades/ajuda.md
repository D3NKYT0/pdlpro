...
| 45 segundos sem interação | Rotina ambient: fala → caminha pelos cômodos desbloqueados → dorme no quarto (cama) ou em pé se o quarto não estiver desbloqueado |
...
Depois de 45 segundos sem conversa, cuidado ou rascunho, o Denkynho inicia uma rotina ambient local ([idleRoutine.ts](../../frontend/src/components/help/idleRoutine.ts)): fala uma frase curta (fora do histórico do chat), caminha com a pose `16-andando` pelos cômodos desbloqueados até o quarto e dorme. A troca de fundo usa `sceneOverride` só na tela — não grava o armário. Se `bedroom` não estiver desbloqueado, após a fala ele dorme em pé na cena atual. Com movimento reduzido ou animação desligada, a caminhada é pulada. Digitar, enviar ou iniciar um cuidado cancela a rotina e restaura a cena equipada. Cada atividade solicitada permanece por oito segundos e todos os timers são liberados ao desmontar a tela.
