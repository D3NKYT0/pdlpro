...
no Compose de produção), fora da árvore que o Nginx serve em `/media/`. Em disco o conteúdo
está cifrado com `PDL_DATA_ENCRYPTION_KEY`; a view de download decifra e devolve `application/gzip`.
O nome inclui um sufixo aleatório, o campo não expõe `url()` e a única saída é essa view, que
confere o token assinado — links expirados ou alterados recebem 403/404 sem corpo. Inclua esse
diretório no backup junto com banco e mídia quando precisar preservar pedidos em aberto. Pacotes
gzip antigos (mágica `\x1f\x8b`) continuam legíveis na transição.
