...
A caça do dia só lê o personagem (`pvp`, `pk`, `online_time`, `level`). A vitrine lê `character_offline_trade` / `character_offline_trade_items` quando o dialeto publica essas consultas, devolve sexo/classe para o retrato da raça e as coordenadas XYZ da loja, e o gateway memoriza a lista por 30 segundos. Se as tabelas não existirem no schema, `/stores` responde `available: false` (200) em vez de 500.
