...
<Button variant="yellow" onClick={highlight}>Destacar</Button>
...
[AchievementIcons](../../frontend/src/components/achievements/AchievementIcons.tsx) reúne as ilustrações das conquistas no mesmo desenho das atividades do Denkynho: degradê, contorno escuro, brilho, sombra de contato e faíscas. `resolveAchievementIconKey` traduz o código do catálogo do backend na arte correspondente, com palavras-chave para códigos de extensões de cliente e a taça como reserva. Cada instância gera IDs próprios de gradiente, então a mesma conquista pode se repetir na tela. Em [AchievementGrid](../../frontend/src/components/AchievementGrid.tsx), a conquista bloqueada mantém a arte como silhueta esmaecida com um cadeado sobreposto, além da borda tracejada e do rótulo de estado.

As artes esmaltadas novas do painel (pacote, carrinho, troca, escudo conferido e estandarte) ficam na área padrão [components/icons](../../frontend/src/components/icons/index.ts). O kit compartilhado (`Glyph`, tons, faíscas) vive em `enamel.tsx`; as conquistas reutilizam esse kit. O caça-níquel da Mesa da Taverna pinta os cinco símbolos (`SlotMark`) com o mesmo kit — a coroa reusa a arte das conquistas. Telas importam de `components/icons` — não pinte Lucide com `color="var(--gold)"` para ocupar o lugar de uma ilustração.
