...
- Issue #2 — Compatibilidade de equipamentos em `items.slot` (Lucera 2): atualizada a consulta `list_character_equipment` no dialeto `lucerav2` para utilizar a coluna `slot` (padrão das distribuições Lucera 2) com contingência automática no gateway para `loc_data AS slot` (`list_character_equipment_fallback`) em schemas legados.
...
- Paridade de consultas SQL entre Lucera 2 e Dream v3: adicionada a consulta `list_character_equipment` ao catálogo `lucerav2` habilitando exibição de itens equipados em inventário, leilões e mercado de personagens; restritas exclusão e decremento de itens a `INVENTORY` e `WAREHOUSE`; e incluído `tax_percent` na consulta de `siege`.
...
e header do `club-v1` no chrome clássico, consolidados em **20 de setembro de 2026**.
