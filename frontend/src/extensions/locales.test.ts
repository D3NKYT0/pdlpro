...
describe('extension locales', () => {
  ...
  it('carrega pt/en/es do skeleton', () => {
    ...
    expect(resources.pt['ext.example']).toMatchObject({ nav: { ping: 'Ping da extensão' } })
    expect(resources.en['ext.example']).toMatchObject({ nav: { ping: 'Extension ping' } })
    expect(resources.es['ext.example']).toMatchObject({ nav: { ping: 'Ping de la extensión' } })
  })
})
