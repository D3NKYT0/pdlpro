...
describe('AdminHubPage', () => {
  it('marca cada módulo com tom de cor para leitura rápida', () => {
    ...
    expect(html).toMatch(/data-tone="content"[\s\S]*Coming Soon/)
    expect(html).toMatch(/href="\/panel\/admin\/coming-soon"/)
    expect(html).toMatch(/data-tone="server"[\s\S]*Painel e servidor/)
    expect(html).toMatch(/href="\/panel\/admin\/server"/)
    expect(html).toMatch(/data-tone="server"[\s\S]*Moderação/)
