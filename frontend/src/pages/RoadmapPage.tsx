    ...
    <div className="program-page program-public">
      <header className="card program-hero">
        <div>
          ...
          <p>
            Acompanhe o que estamos preparando, o que está em desenvolvimento e
            as novidades já entregues.
          </p>
        </div>
        <Flag />
      </header>
              ...
              .map((entry) => (
                <article className="program-item" key={entry.id}>
                  ...
                  <small>
                    ...
                    {entry.target_date
                      ? `· Previsão ${new Date(`${entry.target_date}T12:00:00`).toLocaleDateString("pt-BR")}`
                      : ""}
                  </small>
                  <Link to={`/roadmap/${entry.id}`} className="character-back">
                    Ver atualização <ArrowUpRight size={16} />
                  </Link>
                </article>
              ))}
        ...
        <article className="card program-section">
          ...
          {entry.target_date && (
            <p className="muted">
              ...
              {new Date(`${entry.target_date}T12:00:00`).toLocaleDateString(
                "pt-BR",
              )}
            </p>
          )}
