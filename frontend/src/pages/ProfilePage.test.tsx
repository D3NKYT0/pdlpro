...
it('apresenta rótulos, contador e recusa de avatar no idioma ativo', async () => {
  ...
  expect(screen.getByRole('heading', { name: 'Herói' })).toBeVisible()
  expect(screen.getByText('Good to see you again')).toBeVisible()
  expect(screen.getByRole('list', { name: 'Your journey here' })).toBeVisible()
  expect(screen.getByRole('link', { name: /Account & security/ })).toHaveAttribute('href', '/panel/security')
  expect(screen.queryByText('No story here yet. Write a little about your journey.')).not.toBeInTheDocument()
