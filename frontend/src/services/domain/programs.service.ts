...
export const programsApi = {
  ...
  toggleResource: (id: string, enabled: boolean) =>
    send<Resource>(`/staff/resources/${id}/`, { enabled }, "PATCH"),
  roadmap: (staff = false) =>
    request<RoadmapEntry[]>(`/${staff ? "staff" : "public"}/roadmap/`),
  roadmapDetail: (id: string) =>
    request<RoadmapEntry>(`/public/roadmap/${id}/`),
  ...
  staffSupporters: () =>
    request<{ supporters: Supporter[]; payouts: Payout[] }>(
      "/staff/supporters/",
    ),
  reviewSupporter: (id: string, data: unknown) =>
    send(`/staff/supporters/${id}/`, data, "PATCH"),
  reviewPayout: (id: string, status: string) =>
    send(`/staff/supporter-payouts/${id}/`, { status }, "PATCH"),
  ...
  battleAction: (action: string, entry_id?: string, enabled?: boolean) =>
    send<BattleDetails>("/customer/games/battle-pass/details/", {
      action,
      entry_id,
      enabled,
    }),
  ...
  buyBait: (bait_id: string, quantity: number) =>
    send("/customer/games/fishing/details/", { bait_id, quantity }),
  stats: (kind: string) =>
    request<GameStats>(`/customer/games/statistics/${kind}/`),
  configs: (kind: string) =>
    request<ConfigRow[]>(`/staff/game-content/${kind}/`),
