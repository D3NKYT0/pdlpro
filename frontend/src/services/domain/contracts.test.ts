...
const contracts: Contract[] = [
  ...
  ['auth.sessions', () => authApi.sessions(), '/auth/sessions/'],
  ['auth.revokeSession', () => authApi.revokeSession('session-id'), '/auth/sessions/session-id/', 'DELETE'],
  ['auth.revokeOtherSessions', () => authApi.revokeOtherSessions(), '/auth/sessions/revoke-others/', 'POST'],
  ['auth.me', () => authApi.me(), '/shared/me/'],
  ['auth.acceptTerms', () => authApi.acceptTerms({ terms_accepted: true }), '/shared/me/accept-terms/', 'POST', { terms_accepted: true }],
  ...
  ['auth.exportData', () => authApi.exportData(), '/shared/me/export-data/', 'POST', {}],
  ['auth.requestDeleteCode', () => authApi.requestDeleteCode(), '/shared/me/request-delete-code/', 'POST', {}],
  ['auth.deleteAccount', () => authApi.deleteAccount('123456'), '/shared/me/delete-account/', 'POST', { code: '123456' }],
  ...
  ['wallet.transactions.page', () => walletApi.transactions({ page: 2, page_size: 10 }), '/shared/wallet/transactions/?page=2&page_size=10'],
  ...
  ['auction.create', () => auctionApi.create({ inventory_id: 'inv', item_id: 57, quantity: 2, enchant: 0, min_bid: '10', hours: 24 }), '/customer/auctions/', 'POST', { kind: 'item', inventory_id: 'inv', item_id: 57, quantity: 2, enchant: 0, min_bid: '10', hours: 24 }],
  ['auction.createCharacter', () => auctionApi.create({ kind: 'character', char_id: 7, min_bid: '25', hours: 24 }), '/customer/auctions/', 'POST', { kind: 'character', char_id: 7, min_bid: '25', hours: 24 }],
  ...
  ['content.newsEnglish', () => contentApi.news('en'), '/public/news/?lang=en'],
  ...
  ['content.assistantReplyScreen', () => contentApi.assistantReply('Where is my account?', 'en', undefined, undefined, '/panel/wallet'), '/shared/content/assistant/reply/', 'POST', { message: 'Where is my account?', language: 'en', screen: '/panel/wallet' }],
  ['content.denkynho', () => contentApi.denkynho(), '/shared/content/assistant/pet/'],
  ['content.updateDenkynhoPreferences', () => contentApi.updateDenkynhoPreferences({ preferred_name: 'Dani', detail: 'brief' }), '/shared/content/assistant/pet/', 'PATCH', { preferred_name: 'Dani', detail: 'brief' }],
  ...
  ['content.calendarEnglish', () => contentApi.calendar('en'), '/public/calendar/?lang=en'],
  ['content.legal', () => contentApi.legal(), '/public/legal/'],
  ['content.document', () => contentApi.legalDocument('terms'), '/public/legal/terms/'],
  ['content.legalHistory', () => contentApi.legalHistory(), '/public/legal/history/'],
  ...
  ['games.cast', () => gamesApi.cast('bait'), '/customer/games/fishing/', 'POST', { bait_id: 'bait' }],
  ['games.economy', () => gamesApi.economy(), '/customer/games/economy/'],
  ['games.fight', () => gamesApi.fight('monster'), '/customer/games/economy/monster/fight/', 'POST', { strike: false }],
  ['games.fightStrike', () => gamesApi.fight('boss', { strike: true }), '/customer/games/economy/boss/fight/', 'POST', { strike: true }],
  ...
  ['program.roadmapEnglish', () => programsApi.roadmap(false, 'en'), '/public/roadmap/?lang=en'],
  ...
  ['program.detailEnglish', () => programsApi.roadmapDetail('entry', 'en'), '/public/roadmap/entry/?lang=en'],
  ...
  ['staff.walletPromo', () => staffApi.walletPromo(), '/staff/wallet-promo/'],
  ['staff.saveWalletPromo', () => staffApi.saveWalletPromo({ percent: '15', active: true }), '/staff/wallet-promo/', 'PUT', { percent: '15', active: true }],
  ['staff.coinPackages', () => staffApi.coinPackages(), '/staff/coin-packages/'],
  ['staff.createCoinPackage', () => staffApi.saveCoinPackage({ name: 'Pack' }), '/staff/coin-packages/', 'POST', { name: 'Pack' }],
  ['staff.updateCoinPackage', () => staffApi.saveCoinPackage({ id: 'pkg', active: false }), '/staff/coin-packages/', 'PUT', { id: 'pkg', active: false }],
  ['staff.deleteCoinPackage', () => staffApi.deleteCoinPackage('pkg'), '/staff/coin-packages/', 'DELETE', { id: 'pkg' }],
  ...
  ['staff.calendar', () => staffApi.calendar(), '/staff/calendar/'],
  ['staff.createCalendar', () => staffApi.saveCalendar({ title: 'Siege' }), '/staff/calendar/', 'POST', { title: 'Siege' }],
  ['staff.updateCalendar', () => staffApi.saveCalendar({ id: 'event', is_published: false }), '/staff/calendar/', 'PUT', { id: 'event', is_published: false }],
  ['staff.deleteCalendar', () => staffApi.deleteCalendar('event'), '/staff/calendar/', 'DELETE', { id: 'event' }],
  ['staff.faq', () => staffApi.faq(), '/staff/faq/'],
  ['staff.createFaq', () => staffApi.saveFaq({ question: 'How?' }), '/staff/faq/', 'POST', { question: 'How?' }],
  ['staff.updateFaq', () => staffApi.saveFaq({ id: 'faq', is_published: false }), '/staff/faq/', 'PUT', { id: 'faq', is_published: false }],
  ['staff.deleteFaq', () => staffApi.deleteFaq('faq'), '/staff/faq/', 'DELETE', { id: 'faq' }],
  ['staff.wiki', () => staffApi.wiki(), '/staff/wiki/'],
  ['staff.createWiki', () => staffApi.saveWiki({ title: 'Guide' }), '/staff/wiki/', 'POST', { title: 'Guide' }],
  ['staff.updateWiki', () => staffApi.saveWiki({ id: 'wiki', is_published: false }), '/staff/wiki/', 'PUT', { id: 'wiki', is_published: false }],
  ['staff.deleteWiki', () => staffApi.deleteWiki('wiki'), '/staff/wiki/', 'DELETE', { id: 'wiki' }],
  ['staff.downloads', () => staffApi.downloads(), '/staff/downloads/'],
  ['staff.createDownload', () => staffApi.saveDownload({ title: 'Client' }), '/staff/downloads/', 'POST', { title: 'Client' }],
  ['staff.updateDownload', () => staffApi.saveDownload({ id: 'file', is_published: false }), '/staff/downloads/', 'PUT', { id: 'file', is_published: false }],
  ['staff.deleteDownload', () => staffApi.deleteDownload('file'), '/staff/downloads/', 'DELETE', { id: 'file' }],
  ['staff.notifications', () => staffApi.notifications(), '/staff/notifications/'],
  ['staff.notificationsSearch', () => staffApi.notifications('a & b'), '/staff/notifications/?q=a%20%26%20b'],
  ['staff.sendNotification', () => staffApi.sendNotification({ title: 'Hello', username: 'hero' }), '/staff/notifications/', 'POST', { title: 'Hello', username: 'hero' }],
  ['staff.deleteNotification', () => staffApi.deleteNotification('note'), '/staff/notifications/', 'DELETE', { id: 'note' }],
  ...
  ['staff.secretsStatus', () => staffApi.secretsStatus(), '/staff/secrets/'],
  [
    'staff.secretsAction',
    () => staffApi.secretsAction({ kind: 'revoke_all_sessions', confirmation: 'painel.example.com' }),
    '/staff/secrets/actions/',
    'POST',
    { kind: 'revoke_all_sessions', confirmation: 'painel.example.com' },
  ],
  ['staff.secretsApplyJob', () => staffApi.secretsApplyJob('job-1'), '/staff/secrets/jobs/job-1/apply/', 'POST'],
  ['staff.integrationsStatus', () => staffApi.integrationsStatus(), '/staff/integrations/'],
  [
    'staff.saveIntegrationSection',
    () => staffApi.saveIntegrationSection('smtp', { SMTP_HOST: 'mail.test' }),
    '/staff/integrations/smtp/',
    'PATCH',
    { SMTP_HOST: 'mail.test' },
  ],
  ['staff.testIntegrationSection', () => staffApi.testIntegrationSection('smtp'), '/staff/integrations/smtp/test/', 'POST', {}],
  [
    'staff.testIntegrationSection.email',
    () => staffApi.testIntegrationSection('smtp', { to_email: 'a@b.dev' }),
    '/staff/integrations/smtp/test/',
    'POST',
    { to_email: 'a@b.dev' },
  ],
