...
export function EmptyState({ children, icon = <CircleDashed size={28} aria-hidden="true" />, className = '' }: { children: ReactNode; icon?: ReactNode; className?: string }) {
  return <div className={`ui-empty ${className}`.trim()}>{icon}<p>{children}</p></div>
}
export function LoadingState({ children = 'Carregando informações…', className = '' }: { children?: ReactNode; className?: string }) {
  return <div className={`ui-empty ${className}`.trim()} role="status" aria-live="polite"><Clock3 size={24} aria-hidden="true" /><p>{children}</p></div>
}
export function ErrorNotice({ error, fallback = 'Não foi possível concluir. Tente novamente.', onRetry, className = '' }: { error: unknown; fallback?: string; onRetry?: () => void; className?: string }) {
  ...
  return <div className={`ui-error ${className}`.trim()} role="alert"><AlertCircle size={18} aria-hidden="true" /><span>{error instanceof Error ? error.message : fallback}</span>{onRetry && <Button variant="ghost" onClick={onRetry}>Tentar novamente</Button>}</div>
}
