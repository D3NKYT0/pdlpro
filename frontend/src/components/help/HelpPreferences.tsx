...
export function HelpPreferences({ userId, language, value, disabled, onApply }: {
  ...
  return <section className="help-preferences" aria-label={pt ? 'Preferências da conversa' : 'Conversation preferences'}>
    ...
    <div className="help-preference-actions"><Button size="sm" disabled={disabled} onClick={() => apply(draft)}>{pt ? 'Aplicar preferências' : 'Apply preferences'}</Button><Button size="sm" variant="secondary" disabled={disabled} onClick={() => apply({ ...defaultHelpPreferences, language })}>{pt ? 'Apagar preferências' : 'Erase preferences'}</Button></div>
