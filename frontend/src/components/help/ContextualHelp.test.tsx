...
it('orienta na tela, encaminha o contexto e fecha com Escape sem enviar mensagem', async () => {
  ...
  mount(<ContextualHelp path="/painel/wallet" resources={[]} />)
...
it('mostra consulta e falha de recursos, retira atalhos indisponíveis e não oferece tela desconhecida', async () => {
  const user = userEvent.setup()
  const { rerender } = mount(<ContextualHelp path="/painel/wallet" loading />)
  ...
  rerender(
    <I18nextProvider i18n={i18n}>
      <MemoryRouter><ContextualHelp path="/painel/wallet" error={new Error('failed')} /></MemoryRouter>
    </I18nextProvider>,
  )
  expect(screen.getByRole('alert')).toBeVisible()
  rerender(
    <I18nextProvider i18n={i18n}>
      <MemoryRouter><ContextualHelp path="/painel/wallet" resources={[{ code: 'wallet', enabled: false }]} /></MemoryRouter>
    </I18nextProvider>,
  )
...
it('mostra o mascote, o aviso de necessidade e o chamado pré-preenchido sem enviar o chat', async () => {
  ...
  mount(<ContextualHelp path="/painel/wallet" resources={[]} pet={hungry} />)
...
it('traduz o cantinho quando o idioma do site muda', async () => {
  ...
  mount(<ContextualHelp path="/painel/admin" resources={[]} user={{ role: 'admin', is_staff: true }} />)
