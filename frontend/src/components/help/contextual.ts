...
const topics: Topic[] = [
  ...
  {
    path: '/panel/accounts', title: copy('Contas e personagens', 'Accounts and characters'), action: copy('Abrir minhas contas', 'Open my accounts'), resource: 'accounts',
    tips: [
    ...
    suggestion: copy('Como encontro minhas contas L2 e meus personagens?', 'How do I find my L2 accounts and characters?'), related: ['/panel/inventory', '/panel/security'],
  },
  {
    path: '/panel/wallet', title: copy('Carteira', 'Wallet'), action: copy('Abrir minha carteira', 'Open my wallet'), resource: 'wallet',
    tips: [
    ...
    suggestion: copy('Como acompanho meu saldo e o status de uma compra de moedas?', 'How do I check my balance and the status of a coin purchase?'), related: ['/panel/wallet/orders', '/panel/wallet/statement', '/panel/wallet/game', '/panel/support'],
  },
  {
    path: '/panel/wallet/orders', title: copy('Pedidos de recarga', 'Top-up orders'), action: copy('Abrir pedidos', 'Open orders'), resource: 'wallet',
    tips: [
    ...
    suggestion: copy('Como vejo o status e o detalhe de um pedido de moedas?', 'How do I see the status and detail of a coin order?'), related: ['/panel/wallet', '/panel/wallet/statement', '/panel/support'],
  },
  {
    path: '/panel/wallet/statement', title: copy('Extrato da carteira', 'Wallet statement'), action: copy('Abrir extrato', 'Open statement'), resource: 'wallet',
    tips: [
    ...
    suggestion: copy('Como leio uma movimentação no extrato da carteira?', 'How do I read a movement on the wallet statement?'), related: ['/panel/wallet', '/panel/wallet/orders', '/panel/support'],
  },
  {
    path: '/panel/wallet/game', title: copy('Troca para o jogo', 'Game exchange'), action: copy('Abrir troca para o jogo', 'Open game exchange'), resource: 'wallet',
    tips: [
    ...
    suggestion: copy('Como confiro o destino e a prévia da troca para o jogo?', 'How do I check the destination and preview of a game exchange?'), related: ['/panel/accounts', '/panel/wallet'],
  },
  {
    path: '/panel/inventory', title: copy('Inventário', 'Inventory'), action: copy('Abrir meu inventário', 'Open my inventory'), resource: 'inventory',
    tips: [
    ...
    suggestion: copy('Qual é a diferença entre o inventário dos personagens e a mochila de recompensas?', 'What is the difference between character inventory and the reward bag?'), related: ['/panel/accounts', '/panel/support'],
  },
  {
    path: '/panel/shop', title: copy('Loja', 'Shop'), action: copy('Abrir a loja', 'Open the shop'), resource: 'shop',
    tips: [
    ...
    suggestion: copy('Como confiro os itens e o total da minha compra na loja?', 'How do I check the items and total of my shop purchase?'), related: ['/panel/wallet', '/panel/inventory'],
  },
  {
    path: '/panel/marketplace', title: copy('Marketplace', 'Marketplace'), action: copy('Abrir marketplace', 'Open marketplace'), resource: 'marketplace',
    tips: [
    ...
    suggestion: copy('O que devo conferir antes de comprar ou anunciar no marketplace?', 'What should I check before buying or listing on the marketplace?'), related: ['/panel/inventory', '/panel/wallet'],
  },
  {
    path: '/panel/auctions', title: copy('Leilões', 'Auctions'), action: copy('Abrir leilões', 'Open auctions'), resource: 'auction',
    tips: [
    ...
    suggestion: copy('Como acompanho um leilão e confiro meu lance?', 'How do I follow an auction and check my bid?'), related: ['/panel/wallet', '/panel/support'],
  },
  {
    path: '/panel/games', title: copy('Jogos', 'Games'), action: copy('Abrir jogos', 'Open games'), resource: 'games',
    tips: [
    ...
    suggestion: copy('Onde confiro as regras dos jogos e as recompensas recebidas?', 'Where do I check game rules and received rewards?'), related: ['/panel/inventory', '/panel/rewards'],
  },
  {
    path: '/panel/rewards', title: copy('Jornada e recompensas', 'Journey and rewards'), action: copy('Abrir minhas recompensas', 'Open my rewards'), resource: 'games',
    tips: [
  ...
  },
  {
    ...
    suggestion: copy('Como acompanho meu cadastro e minhas comissões de apoiador?', 'How do I track my supporter application and commissions?'), related: ['/panel/support'],
  },
  {
    path: '/panel/profile', title: copy('Meu perfil', 'My profile'), action: copy('Abrir meu perfil', 'Open my profile'), resource: 'profile',
    tips: [
    ...
    suggestion: copy('Como atualizo meu perfil e encontro as opções de segurança?', 'How do I update my profile and find security options?'), related: ['/panel/security'],
  },
  {
    path: '/panel/security', title: copy('Conta e segurança', 'Account and security'), action: copy('Abrir segurança da conta', 'Open account security'),
    tips: [
  ...
  },
  {
    ...
    suggestion: copy('Como uso a ajuda do Denkynho nesta tela?', 'How do I use Denkynho help on this screen?'), related: ['/panel/support'],
  },
  {
    path: '/panel/admin', title: copy('Administração', 'Administration', 'Administración'), action: copy('Abrir administração', 'Open administration', 'Abrir administración'), staff: true,
    tips: [
      copy(
...
function resolveTopic(path: string | null | undefined) {
  ...
  const canonical = /^\/panel\/accounts\/[a-zA-Z0-9_-]+\/[0-9]+$/.test(path) ? '/panel/accounts' : adminPages.has(path) ? '/panel/admin' : path
...
export function supportTicketPrefill(path: string | null | undefined, language: HelpLanguage = 'pt'): { to: string; label: string } | null {
  ...
  return {
    to: `/panel/support?${params.toString()}`,
...
export function getHelpActionsForText(text: string, user: HelpIdentity = null, resources?: HelpResources, language: HelpLanguage = 'pt'): HelpContext['actions'] {
  ...
  const paths = text.matchAll(/(?:^|[\s(`])(?<path>\/panel(?:\/[a-z]+)*)(?=$|[\s),.;!`])/g)
