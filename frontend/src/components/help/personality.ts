...
export const DENKYNHO_WELCOME = 'Olá! Eu sou o Denkynho, seu companheiro no PDL. Conte sua dúvida ou escolha uma pergunta abaixo. Vamos encontrar o caminho juntos!'
...
const replies: Array<{ matches: RegExp; reply: PersonalityReply }> = [
  {
    ...
    reply: { text: 'Estou bem e com energia para ajudar! E você, como está? Se quiser, pode me contar sua dúvida sobre o PDL.', pose: '02-sucesso' },
  },
  {
    matches: /^(oi|ola|e ai|opa|salve|hey|bom dia|boa tarde|boa noite)( denkynho)?$/,
    reply: { text: 'Olá! Eu sou o Denkynho. É muito bom ter você por aqui! Como posso ajudar na sua jornada pelo PDL?', pose: '01-boas-vindas' },
  },
  {
    matches: /^(obrigado|obrigada|obg|valeu|agradecido|agradecida|muito obrigado|muito obrigada)$/,
    reply: { text: 'Pode contar comigo! Fico feliz em acompanhar você nessa jornada.', pose: '02-sucesso' },
  },
  {
    matches: /^(tchau|ate mais|ate logo|falou|fui)$/,
    reply: { text: 'Até a próxima! Estarei por aqui quando você precisar. Boa jornada!', pose: '01-boas-vindas' },
  },
  {
...
export function matchPersonality(message: string): PersonalityReply | undefined {
  const normalized = normalizeConversation(message)
  if (!normalized || normalized.length > 90) return undefined
  return replies.find(item => item.matches.test(normalized))?.reply
}
