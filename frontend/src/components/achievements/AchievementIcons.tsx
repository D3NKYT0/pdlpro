...
export type AchievementIconProps = Omit<SVGProps<SVGSVGElement>, 'children'>

/**
 * Ilustrações das conquistas no mesmo padrão dos ícones do Denkynho:
 * peças esmaltadas com degradê, contorno escuro, brilho e faíscas.
 * Cada ícone gera IDs próprios de gradiente para poder repetir na mesma tela.
 */
function Glyph({ children, className = '', ...props }: AchievementIconProps & { children: ReactNode }) {
  return (
    <svg
      viewBox="0 0 64 64"
      width="44"
      ...
      className={`achievement-glyph ${className}`.trim()}
      {...props}
    >
      {children}
    </svg>
...
function Ramp({ id, tone, x1 = 14, y1 = 7, x2 = 50, y2 = 57 }: { id: string; tone: Tone; x1?: number; y1?: number; x2?: number; y2?: number }) {
...
function Shadow({ cx = 32, cy = 58, rx = 20, ry = 3 }: { cx?: number; cy?: number; rx?: number; ry?: number }) {
  return <ellipse cx={cx} cy={cy} rx={rx} ry={ry} fill="#130E0C" opacity=".28" />
}

/** Faísca de quatro pontas usada como acento de brilho. */
function Sparkle({ x, y, s = 1, fill = '#FFF3AE', ink = '#D89A25' }: { x: number; y: number; s?: number; fill?: string; ink?: string }) {
  const a = 3.6 * s
  const b = 1.25 * s
  return (
    <path
...
/**
 * Prefixo exclusivo por instância. O contador complementa `useId` porque a mesma
 * conquista pode aparecer em árvores React distintas do documento.
 */
function useGlyphIds() {
  const reactId = useId().replace(/[^a-zA-Z0-9]/g, '')
  const [serial] = useState(() => (glyphSequence += 1))
  const prefix = `ach${reactId}s${serial}`
  return {
