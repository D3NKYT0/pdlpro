...
it('distingue tentativa, vitória, pico e derrota', () => {
  ...
  expect(enchantRevealKind({ attempting: false, success: true, from: 9, level: 10 })).toBe('peak')
  expect(isEnchantPeak(true, false, 9, 10)).toBe(true)
  expect(isEnchantPeak(true, false, 9, 0)).toBe(false)
  expect(isEnchantPeak(true, true, 9, 10)).toBe(false)
