...
class DjangoPreviewSeedService(IPreviewSeedService):
    """Adaptador ORM do seed de preview; só deve rodar com ``PDL_QA_PREVIEW``."""
    ...
    def seed(self, password: str) -> None:
        ...
        for title, status, progress, category in [
            ...
            RoadmapEntry.objects.get_or_create(
                ...
                defaults={
                    ...
                    "category": category,
                    "description": (
                        "Novos desafios e recompensas para os guerreiros de Aden. "
                        "Acompanhe os próximos testes e participe da evolução do servidor."
                    ),
        ...
        from apps.games.domain.fishing_i18n import BAIT_CONTENT_I18N

        for name, paid_with, price, bonus, description in [
