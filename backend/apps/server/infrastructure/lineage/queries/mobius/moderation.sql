-- Mobius — busca e escritas administrativas de personagens/contas.
...
SELECT /*+ MAX_EXECUTION_TIME(5000) */
    ...
    COALESCE(A.access_level, 0) AS account_access,
...
AND (
    :banned_filter < 0
    OR (:banned_filter = 1 AND COALESCE(A.access_level, 0) < 0)
    OR (:banned_filter = 0 AND COALESCE(A.access_level, 0) >= 0)
)
