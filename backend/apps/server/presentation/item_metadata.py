...
def with_item_metadata(value, catalog: IItemCatalog):
    if isinstance(value, (list, tuple)):
        return [with_item_metadata(row, catalog) for row in value]
    if not isinstance(value, dict):
        return value
    result = {key: with_item_metadata(row, catalog) for key, row in value.items()}
    ...
    if not 0 < int(item_id) <= 2147483647:
        return result
    metadata = catalog.metadata(int(item_id))
...
class ItemCatalogAPIView(InjectedAPIView):
    ...
    def finalize_response(self, request, response, *args, **kwargs):
        if 200 <= response.status_code < 300 and hasattr(response, "data"):
            response.data = with_item_metadata(response.data, self.resolve(IItemCatalog))
